...
void BigNumber::AddBigNumbers(BigNumber B1, BigNumber B2){
	...
	while (ok1 || ok2)
	{
		...
		else carry = int(partialres / 10);
		ok1 = B1.l.next_item(digit1);
		ok2 = B2.l.next_item(digit2);
		while (ok1 && !ok2){
			ok1 = B1.l.next_item(digit1);
			if (carry == 0)l.add_to_front(digit1);
			if (carry!=0)
			l.add_to_front((digit1+carry)%10);
		}
...
int main(int argc, char ** argv) {
	...
	int stringseq = 0;
	ifstream input_file;
	argv[1] = "C:/Users/zhenli/Desktop/159201/Assignments/4/samplenumbers2.txt";
	//if (argc == 2) 
	...
	////print the result
	cout << "result is shown below: " << endl;
	RES.PrintBigNumber();
	cout << endl;
	system("pause");
	return 0;
